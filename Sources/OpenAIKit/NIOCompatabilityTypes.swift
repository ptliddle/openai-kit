...
#if !USE_NIO
// If we're not using NIO define the HTTPMethod so we don't rely on NIO
...
public class HTTPHeaders: Sequence {
    ...
    public func makeIterator() -> some IteratorProtocol<(String, String)> {
        return headers.map({ ($0.key, $0.value) }).makeIterator()
    }
}
#endif
