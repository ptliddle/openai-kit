...
public class JsonSchemaCreator {
    ...
    public static func createJSONSchema<T: Codable>(for object: T, id: String? = nil, schema: String? = "http://json-schema.org/draft-07/schema#") -> JSONSchema {
        _createJSONSchema(for: object, id: id, schema: schema)
    }
     
    private static func _createJSONSchema<T: Codable>(for object: T, id: String? = nil, schema: String? = nil) -> JSONSchema {
        ...
        for child in mirror.children {
            ...
            let subjectType = type(of: child.value)
            
            switch subjectType {
            case is String.Type:
                properties[label] = JSONSchema(type: .string)
            case is Int.Type, is Int8.Type, is Int16.Type, is Int32.Type, is Int64.Type:
                properties[label] = JSONSchema(type: .integer)
            case is Float.Type, is Double.Type:
                properties[label] = JSONSchema(type: .number)
            case is Bool.Type:
                properties[label] = JSONSchema(type: .boolean)
            case is Optional<Any>.Type:
                properties[label] = JSONSchema(type: .null)
            default:
                if let codArray = child.value as? [Codable] {
                   guard let arrayElement = codArray.first else { continue }
                   properties[label] = JSONSchema(type: .array,
                                                  items: _createJSONSchema(for: arrayElement))
                }
                else if let codValue = child.value as? Codable {
                    properties[label] = _createJSONSchema(for: codValue)
                }
